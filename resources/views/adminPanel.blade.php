                ...
                <form class="max-w-md mx-auto" method="POST" action="{{ route('NewPoster') }}"
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                        <input type="text" name="name" id="name"
                            ...
                            required />
                        <label for="name"
                            class="peer-focus:font-medium absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 
                            -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-black 
                            peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Название</label>
                    </div>
                    <div class="relative z-0 w-full mb-5 group">
                    ...
                    <div class="relative z-0 w-full mb-5 group">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach ($genres as $genre)
                                <button type="button" @click="selectedGenres.includes({{ $genre->id }}) ?
                                 selectedGenres = selectedGenres.filter(g => g !== {{ $genre->id }}) : selectedGenres.push({{ $genre->id }})" 
                                 :class="{ 'bg-blue-500 text-white': selectedGenres.includes({{ $genre->id }}), 'bg-gray-200 text-gray-700': !selectedGenres.includes({{ $genre->id }}) }" 
                                 class="px-3 py-1 rounded-full transition duration-300 ease-in-out hover:bg-blue-500 hover:text-white">{{ $genre->name }}</button>
                            @endforeach
                        </div>
